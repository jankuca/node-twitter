...
/**
 * @constructor
 * @param {Object.<string, *>=} options Twitter client object.
 */
function Twitter(options) {
	...
	this.$oauth = options.oauth || new OAuth(options);
	this.$keygrip = null;
	if (options.cookie_secret !== null) {
		this.$keygrip = new Keygrip([options.cookie_secret]);
	}
...
Twitter.prototype.createChild = function (option_extension) {
	var twitter = Object.create(this);

	var options = Object.create(this.options_);
	Object.keys(option_extension || {}).forEach(function (key) {
		options[key] = option_extension[key];
	});
	twitter.options_ = options;
...
Twitter.prototype.get = function(url, params, callback) {
	...
	if (url.charAt(0) == '/')
		url = this.options_.rest_base + url;

	url = params == null ? url : url + '?' + querystring.stringify(params);

	this.$oauth.get(url,
		this.options_.access_token,
		this.options_.access_token_secret,
...
Twitter.prototype.search = function(q, params, callback) {
	...
	var url = this.options_.search_base + '/tweets.json';
...
Twitter.prototype.stream = function(method, params, callback) {
	...
	var stream_base = this.options_.stream_base;

	// Stream type customisations
	if (method === 'user') {
		stream_base = this.options_.user_stream_base;
		...
		stream_base = this.options_.site_stream_base;
		...
		stream_base = this.options_.filter_stream_base;
		// Workaround for node-oauth vs. twitter commas-in-params bug
		if ( params && params.track && Array.isArray(params.track) ) {
			params.track = params.track.join(',');
		}
...
Twitter.prototype.login = function(mount, success) {
	...
	this.options_.login_mount = mount = mount || '/twauth';

	// Use secure cookie if forced to https and haven't configured otherwise
	if ( this.options_.secure && !this.options_.cookie_options.secure )
		this.options_.cookie_options.secure = true;
	...
	return function handle(req, res, next) {
		...
		if ( !self.$oauth._authorize_callback ) {
			...
			var scheme = (req.socket.secure || self.options_.secure) ? 'https://' : 'http://';
		...
		// Returning from Twitter with oauth_token
			...
			function(error, access_token, access_token_secret, params) {
				...
				if ( error ) {
					...
					// store access token
					self.options_.access_token = twauth.access_token;
					self.options_.access_token_secret = twauth.access_token_secret;
					cookies.set(self.options_.cookie, JSON.stringify({
						user_id: user_id,
						screen_name: screen_name,
						access_token: access_token,
						access_token_secret: access_token_secret
					}), self.options_.cookie_options);
		...
		// Begin OAuth transaction if we have no cookie or access_token_secret
			...
			function(error, oauth_token, oauth_token_secret, oauth_authorize_url, params) {
				if ( error ) {
					...
					res.writeHead(302, {
						'Location': self.options_.authorize_url + '?' +
		...
		// FIXME: this is dumb
			...
			cookies.set(self.options_.cookie, null, self.options_.cookie_options);
...
Twitter.prototype.gatekeeper = function(options) {
	var self = this,
		mount = this.options_.login_mount || '/twauth',
        defaults = {
...
Twitter.prototype.userProfileImage = function(id, params, callback) {
	...
	var request = this.$oauth.get(this.options_.rest_base + url,
		this.options_.access_token,
		this.options_.access_token_secret);
	request.on('response', function(response) {
...
Twitter.prototype._readCookie = function(cookies) {
	...
	try {
		return JSON.parse(cookies.get(this.options_.cookie));
	} catch (error) {
		return null;
	}
};
